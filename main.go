...
func main() {
	...
	//Test Operations in G2
	fmt.Printf("Input the parameters from off-chain\n")
	auth14 := utils.Transact(client, privatekey, big.NewInt(0))
	tx14, _ := Contract.TestG2Add(auth14, Convert.G2ToG2Point(PK.Tau2[0]), Convert.G2ToG2Point(PK.Tau2[1]))
	receipt14, err := bind.WaitMined(context.Background(), client, tx14)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Add operation in G2 Gas used: %d\n", receipt14.GasUsed)

	auth15 := utils.Transact(client, privatekey, big.NewInt(0))
	tx15, _ := Contract.TestG2Mul(auth15, big.NewInt(int64(1)), Convert.G2ToG2Point(PK.Tau2[1]))
	receipt15, err := bind.WaitMined(context.Background(), client, tx15)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Mul operation in G2 Gas used: %d\n", receipt15.GasUsed)

	auth16 := utils.Transact(client, privatekey, big.NewInt(0))
	tx16, _ := Contract.TestG2Neg(auth16, Convert.G2ToG2Point(PK.Tau2[1]))
	receipt16, err := bind.WaitMined(context.Background(), client, tx16)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Neg operation in G2 Gas used: %d\n", receipt16.GasUsed)

	fmt.Printf("Invoke the parameters from on-chain\n")
	auth17 := utils.Transact(client, privatekey, big.NewInt(0))
	tx17, _ := Contract.TestG1AddNO(auth17)
	receipt17, err := bind.WaitMined(context.Background(), client, tx17)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Add operation in G2 Gas used: %d\n", receipt17.GasUsed)

	auth18 := utils.Transact(client, privatekey, big.NewInt(0))
	tx18, _ := Contract.TestG2MulNO(auth18)
	receipt18, err := bind.WaitMined(context.Background(), client, tx18)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Mul operation in G2 Gas used: %d\n", receipt18.GasUsed)

	auth19 := utils.Transact(client, privatekey, big.NewInt(0))
	tx19, _ := Contract.TestG2NegNO(auth19)
	receipt19, err := bind.WaitMined(context.Background(), client, tx19)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Neg operation in G2 Gas used: %d\n", receipt19.GasUsed)

	//Test Operations in G1
	fmt.Printf("Input the parameters from off-chain\n")
	auth20 := utils.Transact(client, privatekey, big.NewInt(0))
	tx20, _ := Contract.TestG1Add(auth20, Convert.G1ToG1Point(PK.Tau1[0]), Convert.G1ToG1Point(PK.Tau1[1]))
	receipt20, err := bind.WaitMined(context.Background(), client, tx20)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Add operation in G1 Gas used: %d\n", receipt20.GasUsed)

	auth21 := utils.Transact(client, privatekey, big.NewInt(0))
	tx21, _ := Contract.TestG1Mul(auth21, big.NewInt(int64(2)), Convert.G1ToG1Point(PK.Tau1[0]))
	receipt21, err := bind.WaitMined(context.Background(), client, tx21)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Mul operation in G1 Gas used: %d\n", receipt21.GasUsed)

	auth22 := utils.Transact(client, privatekey, big.NewInt(0))
	tx22, _ := Contract.TestG1Neg(auth22, Convert.G1ToG1Point(PK.Tau1[0]))
	receipt22, err := bind.WaitMined(context.Background(), client, tx22)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Neg operation in G1 Gas used: %d\n", receipt22.GasUsed)

	fmt.Printf("Invoke the parameters from on-chain\n")
	auth23 := utils.Transact(client, privatekey, big.NewInt(0))
	tx23, _ := Contract.TestG1AddNO(auth23)
	receipt23, err := bind.WaitMined(context.Background(), client, tx23)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Add operation in G1 Gas used: %d\n", receipt23.GasUsed)
	...
	tx24, _ := Contract.TestG1MulNO(auth24)
	...
	auth25 := utils.Transact(client, privatekey, big.NewInt(0))
	tx25, _ := Contract.TestG1NegNO(auth25)
	receipt25, err := bind.WaitMined(context.Background(), client, tx25)
	if err != nil {
		log.Fatalf("Tx receipt failed: %v", err)
	}
	fmt.Printf("Neg operation in G1 Gas used: %d\n", receipt25.GasUsed)
